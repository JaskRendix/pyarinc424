...
def test_empty_dataframe_returns_empty_list():
    df = make_df([])
    assert df_to_communications(df) == []
...
def test_basic_conversion_single_row():
    ...
    comms = df_to_communications(df)
    assert len(comms) == 1

    c = comms[0]
    assert c.airport_identifier == "LIMC"
...
def test_missing_optional_fields_become_empty_strings_and_none_coords():
    ...
    c = df_to_communications(df)[0]

    assert c.airport_identifier == "LIML"
...
def test_multiple_rows_are_all_converted():
    df = make_df(
        [
            {"AirportIdentifier": "A", "ServiceType": "TWR"},
            {"AirportIdentifier": "B", "ServiceType": "GND"},
            {"AirportIdentifier": "C", "ServiceType": "APP"},
        ]
    )
    ...
    assert len(comms) == 3
    assert [c.airport_identifier for c in comms] == ["A", "B", "C"]
